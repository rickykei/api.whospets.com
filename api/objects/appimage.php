...
class Appimage{
        ...
        if($stmt->execute()){
			...
			$this->id = $this->conn->lastInsertId();
					...
					if ($handle->uploaded) {
					  ...
					  $handle->process('/vhost/sosopet/sosopet/images/app_img/'.$this->app_table.'/'.$this->product_id);
					  //$handle->clean();
					  $handle->file_new_name_body   = $name;
					  $handle->image_resize         = true;
					  $handle->image_x              = 250;
					  $handle->image_ratio_y        = true;
					  $handle->process('/vhost/sosopet/sosopet/images/app_img/'.$this->app_table.'/thumb/'.$this->product_id);
    ...
    function getUserPets(){
        // select all query
        $query = "SELECT
                    shop_products.*,app_image.filename as image
                FROM
                    " . $this->table_name . " , user,shop_store,app_image
                WHERE
					shop_products.store_id=shop_store.id
					and shop_products.product_id = app_image.product_id
	...
	function getPetsByStatus(){
        // select all query
        $query = "SELECT
                    shop_products.*,app_image.title as title,app_image.filename as filename
                FROM
                    " . $this->table_name . " ,app_image, shop_store
                WHERE
					shop_products.store_id=shop_store.id
					and shop_products.product_id = app_image.product_id and
